...
function dictValueParserAdd() {
    return {
        serialize: (src, buidler) => {
            buidler.storeRef((0, ton_core_1.beginCell)().store(storeAdd(src)).endCell());
        },
        parse: (src) => {
...
function dictValueParserSub() {
    return {
        serialize: (src, buidler) => {
            buidler.storeRef((0, ton_core_1.beginCell)().store(storeSub(src)).endCell());
        },
        parse: (src) => {
...
class Functions {
    ...
    async send(provider, via, args, message) {
        ...
        if (message && typeof message === 'object' && !(message instanceof ton_core_1.Slice) && message.$$type === 'Sub') {
            body = (0, ton_core_1.beginCell)().store(storeSub(message)).endCell();
        }
        if (body === null) {
