...
function resolveDescriptors(ctx) {
    ...
    function buildFieldDescription(src, index) {
        ...
        if ((0, isRuntimeType_1.isRuntimeType)(tr)) {
            (0, ast_1.throwError)((0, types_1.printTypeRef)(tr) + ' is a runtime only type and can\'t be used as field', src.ref);
        }
    ...
    function buildConstantDescription(src) {
        let tr = buildTypeRef(src.type, types);
        let d = (0, resolveConstantValue_1.resolveConstantValue)(tr, src.value, ctx);
            ...
            for (const f of a.declarations) {
                if (f.kind === 'def_field') {
                    ...
                    if (f.attributes.find((v) => v.type !== 'overrides')) {
                        (0, ast_1.throwError)(`Constant can be only overriden`, f.ref);
                    }
        ...
        if (a.kind === 'def_trait') {
            for (const f of a.declarations) {
                if (f.kind === 'def_field') {
                    ...
                    if (f.attributes.find((v) => v.type === 'overrides')) {
                        (0, ast_1.throwError)(`Trait constant cannot be overriden`, f.ref);
                    }
    ...
    //
    function resolveFunctionDescriptor(sself, a, origin) {
        let self = sself;
        // Resolve return
        ...
        if (isGetter) {
            for (let arg of args) {
                if ((0, isRuntimeType_1.isRuntimeType)(arg.type)) {
                    (0, ast_1.throwError)((0, types_1.printTypeRef)(arg.type) + ' is a runtime onlye type and can\'t be used as a getter argument', arg.ref);
                }
            }
            if ((0, isRuntimeType_1.isRuntimeType)(returns)) {
                (0, ast_1.throwError)((0, types_1.printTypeRef)(returns) + ' is a runtime onlye type and can\'t be used as getter return type', a.ref);
            }
        }
        // Register function
        return {
    ...
    function resolveInitFunction(ast) {
        ...
        for (let a of args) {
            if ((0, isRuntimeType_1.isRuntimeType)(a.type)) {
                (0, ast_1.throwError)((0, types_1.printTypeRef)(a.type) + ' is a runtime onlye type and can\'t be used as a init function argument', a.ref);
            }
        }
        return {
            args,
        ...
        if (a.kind === 'def_contract' || a.kind === 'def_trait') {
            ...
            for (const d of a.declarations) {
                ...
                if (d.kind === 'def_receive') {
                    ...
                    if (d.selector.kind.startsWith('external-') && !(0, features_1.enabledExternals)(ctx)) {
                        ...
                        if (arg.type.kind === "type_ref_simple") {
                            ...
                            if (arg.type.name === "Slice") {
                                ...
                                if (type.fields.length !== type.partialFieldCount) {
                                    (0, ast_1.throwError)('This message is too big for bounce receiver, you need to wrap it to a bounced<' + arg.type.name + '>.', d.ref);
                                }
